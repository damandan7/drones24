...
def main():
    ...
    with SyncCrazyflie(URI1, cf=Crazyflie(rw_cache='./cache')) as scf1, SyncCrazyflie(URI4, cf=Crazyflie(rw_cache='./cache')) as scf4, SyncCrazyflie(URI6, cf=Crazyflie(rw_cache='./cache')) as scf6, SyncCrazyflie(URI7, cf=Crazyflie(rw_cache='./cache')) as scf7: 
        ...
        hlc1.go_to(.35, 0, 1, 0, 2, relative=False)
        hlc4.go_to(-.35, 0, 1, 0, 2, relative=False)
        hlc6.go_to(0, .35, 1, 0, 2, relative=False)
        hlc7.go_to(0, -.35, 1, 0, 2, relative=False)
        ...
        try:
            ...
            while cap.isOpened():
                ...
                if hand1:
                    ...
                    palm1 = lmList1[9]
                    point3_x = (palm1[0] - center_x) / 110
                    point3_y = 2 - (palm1[1]) / 150
                    #wrist = lmList1[0]          #&&&&&&&&&&
                    #point3_x = (wrist[0] - center_x) / 200    #&&&&&&&&&&
                    #point3_y = -(wrist[1] - center_y) / 200   #&&&&&&&&&&
                    ...
                    if hand_grabbed:
                        ...
                        print("P3:", P3)
                        hlc1.go_to(P3[0]+.35, depth, P3[1], 0, 1, relative=False)
                        hlc4.go_to(P3[0]-.35, depth, P3[1], 0, 1, relative=False)
                        hlc6.go_to(P3[0], depth+.35, P3[1], 0, 1, relative=False)
                        hlc7.go_to(P3[0], depth-.35, P3[1], 0, 1, relative=False)
